...
3. **Record which option the human chose** and that it came from that
   comment. It goes into **whichever comment the resumed path posts first**:
   the "the investigation is continuing, on this basis" comment when the
   investigation goes on, or the close comment itself when the answer is to
   close — `### Closed: deterministic cause, not flaky` for a deterministic
   cause (Step 3's template, `{DECISION_RECORD}` filled in), or, for "close
   as unable to reproduce", this comment — posted with the same
   `gh api "repos/growilabs/growi/issues/{ISSUE_NUMBER}/comments" -X POST -f body="$(cat <<'EOF' … EOF)"`
   form 2-E uses, and only then
   `gh api "repos/growilabs/growi/issues/{ISSUE_NUMBER}" -X PATCH -f state=closed -f state_reason=not_planned`,
   labels untouched:

   ```
   ### Auto-closed: not reproduced within {DAYS_SINCE_NEWEST} days

   - Newest observation: {NEWEST}
   - Closed by: `investigate-flaky-test`, on the human decision in {COMMENT_HTML_URL}

   {DECISION_RECORD}

   Closing as `not planned`: the identity has not recurred since the newest
   observation, and the maintainer decided not to pursue it further. If it
   comes back, `detect-flaky-ci`'s closed-issue path reopens it.

   _Generated by [Claude Code](https://claude.ai/code)_
   ```

   `{NEWEST}` is the newest observation timestamp among the blocks Step 1
   collected (the body's first observation and every `### Additional
   observation` / `### Backfilled observation` comment — the same value the
   routine's 4-B computes), and `{DAYS_SINCE_NEWEST}` is the whole number of
   days from it to now. The routine's `--stale-days` is **not** available
   here and no threshold was applied, so neither appears. Only the heading
   is shared with the routine's 4-E, and that is deliberate: `### Auto-closed:
   not reproduced within ` is what the routine's manual-reopen guard (4-C)
   and its observation-count exclusion (4-B, Step 5) recognize, so a
   human-decided not-reproduced close behaves like an automatic one if the
   identity ever comes back. If `{NEWEST}` cannot be determined, do not
   post and do not close — pause again per **Pausing for a human decision**
   saying so. A
   close answer never produces a "continuing" comment — the close comment is
   the only one posted — so writing the record only into a "continuing"
   comment would drop it entirely on exactly the two answers that end the
   issue.

   Write it as **one paragraph** naming the option the human chose and
   linking their comment URL (`{COMMENT_HTML_URL}` from the invocation
   line). When a PR is opened, the PR body's Root Cause section carries the
   same paragraph as well — the issue comment is not a substitute for it.
   A gate crossed on a human's say-so must not later read as one the
   evidence crossed on its own.
...
Mark as under investigation, same as `investigate-issue`. The phase labels
begin with an emoji, so this is the read-then-whole-array `PATCH` form 6-C
uses (never `gh issue edit`, which is GraphQL-backed and rejected in a cloud
routine's session; never a `DELETE /labels/{name}` with an emoji in the path):

```bash
LABELS_FILE="${TMPDIR:-/tmp}/flaky-phase-labels-{ISSUE_NUMBER}.json"

gh api "repos/growilabs/growi/issues/{ISSUE_NUMBER}" -q '[.labels[].name]' \
  | jq -c --arg rm '{EXACT_PHASE_NEW_LABEL}' \
          --arg add '{EXACT_PHASE_UNDER_INVESTIGATION_LABEL}' \
...
--paginate -q '.[].name'`), as the Precondition requires; the `PATCH`
replaces the whole set, so it must be computed from the labels read
...
{DECISION_RECORD — only on a human-decision resume: one paragraph naming the
option the human chose and linking their comment URL. Omit this line
entirely when this skill reached the close on its own evidence.}
...
`{DECISION_RECORD}` is the one part of the template that is present on some
closes and absent on others: fill it in when a human decision sent this run
down the close path (**Resuming after a human decision**, item 3 — this
comment is then the first one the resumed path posts, so it is where that
record has to live), and leave the line out entirely when the evidence alone
led here. Everything else in the template is always written.
...
- REST (`gh api`) is **the** form every read and write in this skill uses —
  not a fallback to reach for after a failure. `gh issue` / `gh label` /
  `gh pr` subcommands are GraphQL mutations, and a cloud routine's session
  rejects GraphQL ("This GraphQL query is not enabled for this session"),
  so seeing that error means a subcommand slipped into a step: replace that
  call with the `gh api` form the step documents and continue — do not
  retry the subcommand. `detect-flaky-ci`'s Error Handling states the same
  rule for its own writes.
