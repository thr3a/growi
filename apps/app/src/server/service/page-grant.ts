...
export interface IPageGrantService {
  ...
  separateNormalizableAndNotNormalizablePages: (
    user,
    pages,
  ) => Promise<
    [(PageDocument & { _id: any })[], (PageDocument & { _id: any })[]]
  >;
  generateUpdateGrantInfoToOverwriteDescendants: (
    operator,
    updateGrant?: PageGrant,
    grantGroupIds?: IGrantedGroup[],
  ) => Promise<UpdateGrantInfo>;
  canOverwriteDescendants: (
    targetPath: string,
    operator: { _id: ObjectIdLike },
  ...
  getUserRelatedGroups: (
    user?: IUserHasId | HydratedDocument<IUser> | null,
  ) => Promise<PopulatedGrantedGroup[]>;
  getPopulatedGrantedGroups: (
    grantedGroups: IGrantedGroup[],
  ) => Promise<PopulatedGrantedGroup[]>;
  getUserRelatedGrantedGroups: (
    page: PageDocument,
    user,
  ) => Promise<IGrantedGroup[]>;
  getUserRelatedGrantedGroupsSyncronously: (
    userRelatedGroups: PopulatedGrantedGroup[],
    page: PageDocument,
  ) => IGrantedGroup[];
  getNonUserRelatedGrantedGroups: (
    page: PageDocument,
    user,
  ) => Promise<IGrantedGroup[]>;
  isUserGrantedPageAccess: (
    ...
    user,
    userRelatedGroups: PopulatedGrantedGroup[],
    allowAnyoneWithTheLink?: boolean,
  ) => boolean;
...
class PageGrantService implements IPageGrantService {
  ...
  private validateComparableTarget(comparable: ComparableTarget) {
    ...
    if (
      ...
      (grantedUserIds == null || grantedUserIds.length !== 1)
    ) {
  ...
  private validateGrant(
    ...
    ancestor: ComparableAncestor,
    descendants?: ComparableDescendants,
  ): boolean {
    /*
     * the page itself
     */
    ...
    // GRANT_OWNER
      ...
      if (
        ancestor.grantedUserIds[0].toString() !==
        target.grantedUserIds[0].toString()
      ) {
    ...
    // GRANT_USER_GROUP
      ...
      if (
        ancestor.applicableGroupIds == null ||
        ancestor.applicableUserIds == null
      ) {
        throw Error(
          'applicableGroupIds and applicableUserIds are not specified',
        );
      }
      ...
      if (target.grant === Page.GRANT_OWNER) {
        ...
        if (
          !includesObjectIds(ancestor.applicableUserIds, [
            target.grantedUserIds[0],
          ])
        ) {
      ...
      if (target.grant === Page.GRANT_USER_GROUP) {
        if (
          target.grantedGroupIds == null ||
          target.grantedGroupIds.length === 0
        ) {
          throw Error('grantedGroupId must not be empty');
        }
        const targetGrantedGroupStrIds = target.grantedGroupIds.map((e) =>
          typeof e.item === 'string' ? e.item : e.item._id,
        );
        if (
          !includesObjectIds(
            ancestor.applicableGroupIds,
            targetGrantedGroupStrIds,
          )
        ) {
    ...
    // GRANT_OWNER
      ...
      if (
        descendants.grantedGroupIds.length !== 0 ||
        descendants.grantedUserIds.length > 1
      ) {
      ...
      if (
        descendants.grantedUserIds.length === 1 &&
        descendants.grantedUserIds[0].toString() !==
          target.grantedUserIds[0].toString()
      ) {
    ...
    // GRANT_USER_GROUP
      ...
      if (
        target.applicableGroupIds == null ||
        target.applicableUserIds == null
      ) {
        throw Error(
          'applicableGroupIds and applicableUserIds must not be null',
        );
      }
      ...
      const shouldNotExistUserIds = excludeTestIdsFromTargetIds(
        descendants.grantedUserIds,
        target.applicableUserIds,
      );
      if (
        shouldNotExistGroupIds.length !== 0 ||
        shouldNotExistUserIds.length !== 0
      ) {
        return false;
      }
    }
  ...
  async validateGrantChange(
    ...
    return this.validateGrantChangeSyncronously(
      userRelatedGroups,
  ...
  validateGrantChangeSyncronously(
    ...
    if (!userBelongsToAllPreviousGrantedGroups) {
      ...
      const pageGrantIncludesUserRelatedGroup = hasIntersection(
  ...
  private async generateComparableTargetWithApplicableData(
    ...
    if (grant === Page.GRANT_USER_GROUP) {
      ...
      } = divideByType(grantedGroupIds);
      const targetUserGroups = await UserGroup.find({
        _id: { $in: grantedUserGroupIds },
      });
      const targetExternalUserGroups = await ExternalUserGroup.find({
        _id: { $in: grantedExternalUserGroupIds },
      });
      if (
        targetUserGroups.length === 0 &&
        targetExternalUserGroups.length === 0
      ) {
        throw Error('Target user group does not exist');
      }

      const userGroupRelations = await UserGroupRelation.find({
        relatedGroup: { $in: targetUserGroups.map((g) => g._id) },
      });
      const externalUserGroupRelations = await ExternalUserGroupRelation.find({
        relatedGroup: { $in: targetExternalUserGroups.map((g) => g._id) },
      });
      applicableUserIds = Array.from(
        new Set(
          [...userGroupRelations, ...externalUserGroupRelations].map(
            (u) => u.relatedUser as ObjectIdLike,
          ),
        ),
      );
   ...
   * @returns Promise<ComparableAncestor>
   */
  private async generateComparableAncestor(
    targetPath: string,
    includeNotMigratedPages: boolean,
  ): Promise<ComparableAncestor> {
    const Page = mongoose.model<IPage, PageModel>('Page');
    ...
    if (testAncestor.grant === Page.GRANT_USER_GROUP) {
      ...
      const { grantedUserGroups, grantedExternalUserGroups } = divideByType(
        testAncestor.grantedGroups,
      );

      const userGroupRelations = await UserGroupRelation.find(
        { relatedGroup: { $in: grantedUserGroups } },
        { _id: 0, relatedUser: 1 },
      );
      const externalUserGroupRelations = await ExternalUserGroupRelation.find(
        { relatedGroup: { $in: grantedExternalUserGroups } },
        { _id: 0, relatedUser: 1 },
      );
      applicableUserIds = Array.from(
        new Set(
          [...userGroupRelations, ...externalUserGroupRelations].map(
            (r) => r.relatedUser as ObjectIdLike,
          ),
        ),
      );
   ...
   */
  private async generateComparableDescendants(
    targetPath: string,
    user,
    includeNotMigratedPages = false,
  ): Promise<ComparableDescendants> {
    const Page = mongoose.model<IPage, PageModel>('Page');
    ...
    // GRANT_OWNER group
    const grantOwnerResult = result.filter(
      (r) => r._id === Page.GRANT_OWNER,
    )[0]; // users of GRANT_OWNER
    const grantedUserIds: ObjectIdLike[] =
      grantOwnerResult?.grantedUsersSet ?? [];
    // GRANT_USER_GROUP group
    const grantUserGroupResult = result.filter(
      (r) => r._id === Page.GRANT_USER_GROUP,
    )[0]; // users of GRANT_OWNER
  ...
  async isGrantNormalized(
    ...
    const comparableAncestor = await this.generateComparableAncestor(
      targetPath,
      includeNotMigratedPages,
    );
    ...
    const comparableTarget =
      await this.generateComparableTargetWithApplicableData(
        grant,
        grantedUserIds,
        grantedGroupIds,
      );
    const comparableDescendants = await this.generateComparableDescendants(
      targetPath,
      user,
      includeNotMigratedPages,
    );

    return this.validateGrant(
      comparableTarget,
      comparableAncestor,
      comparableDescendants,
    );
  }
   ...
   */
  async separateNormalizableAndNotNormalizablePages(
    user,
    pages,
  ): Promise<
    [(PageDocument & { _id: any })[], (PageDocument & { _id: any })[]]
  ...
  async calcApplicableGrantData(page, user): Promise<IRecordApplicableGrant> {
    ...
    if (isAnyGrantApplicable) {
      ...
      data[PageGrant.GRANT_USER_GROUP] = {
    ...
    if (grant === PageGrant.GRANT_PUBLIC) {
      ...
      const isUserExistInExternalUserGroup = (
        await Promise.all(
          targetExternalUserGroups.map((group) => {
            return ExternalUserGroupRelation.countByGroupIdsAndUser(
              [group._id],
              user,
            );
          }),
      ...
      const isUserExistInGroup =
        isUserExistInUserGroup || isUserExistInExternalUserGroup;
      ...
      const applicableUserGroups = (
        await Promise.all(
          targetUserGroups.map((group) => {
            return UserGroupRelation.findGroupsWithDescendantsByGroupAndUser(
              group,
              user,
            );
          }),
      ...
      const applicableExternalUserGroups = (
        await Promise.all(
          targetExternalUserGroups.map((group) => {
            return ExternalUserGroupRelation.findGroupsWithDescendantsByGroupAndUser(
              group,
              user,
            );
          }),
   ...
   */
  async getPageGroupGrantData(
    page: PageDocument,
    user,
  ): Promise<GroupGrantData> {
    if (isTopPage(page.path)) {
      return { userRelatedGroups: [], nonUserRelatedGrantedGroups: [] };
    }

    const userRelatedGroups = await this.getUserRelatedGroups(user);
    let userRelatedGroupsData: UserRelatedGroupsData[] = userRelatedGroups.map(
      (group) => {
        const provider =
    ...
    const populatedGrantedGroups = await this.getPopulatedGrantedGroups(
      page.grantedGroups,
    );
    ...
    populatedGrantedGroups.forEach((group) => {
      const userRelatedGrantedGroup = userRelatedGroupsData.find(
        (userRelatedGroup) => {
    ...
    const grantedUserIds =
      page.grantedUsers?.map((user) => getIdForRef(user)) ?? [];
    const comparableAncestor = await this.generateComparableAncestor(
      page.path,
      false,
    );
    userRelatedGroupsData = userRelatedGroupsData.map((groupData) => {
      ...
      const groupsToGrant = [
        ...(page.grantedGroups ?? []),
        { item: groupData.id, type: groupData.type },
      ];
      const comparableTarget: ComparableTarget = {
      ...
      const status = this.validateGrant(comparableTarget, comparableAncestor)
        ? UserGroupPageGrantStatus.notGranted
        : UserGroupPageGrantStatus.cannotGrant;
  ...
  async getUserRelatedGroups(
    ...
    const userRelatedUserGroups =
      await UserGroupRelation.findAllGroupsForUser(user);
    const userRelatedExternalUserGroups =
      await ExternalUserGroupRelation.findAllGroupsForUser(user);
    return [
  ...
  async getPopulatedGrantedGroups(
    grantedGroups: IGrantedGroup[],
  ): Promise<PopulatedGrantedGroup[]> {
    const { grantedUserGroups, grantedExternalUserGroups } =
      divideByType(grantedGroups);
    const userGroupDocuments = await UserGroup.find({
      _id: { $in: grantedUserGroups },
    });
    const externalUserGroupDocuments = await ExternalUserGroup.find({
      _id: { $in: grantedExternalUserGroups },
    });
    return [
   ...
   */
  async getUserRelatedGrantedGroups(
    page: PageDocument,
    user,
  ): Promise<IGrantedGroup[]> {
    const userRelatedGroups = await this.getUserRelatedGroups(user);
    return this.getUserRelatedGrantedGroupsSyncronously(
      userRelatedGroups,
      page,
    );
  }
  ...
  getUserRelatedGrantedGroupsSyncronously(
    ...
    const userRelatedGroupIds: string[] = userRelatedGroups.map((ug) =>
      ug.item._id.toString(),
    );
    return (
   ...
   */
  async getNonUserRelatedGrantedGroups(
    page: PageDocument,
    user,
  ): Promise<IGrantedGroup[]> {
    const userRelatedGroups = await this.getUserRelatedGroups(user);
    ...
    return (
      page.grantedGroups?.filter((group) => {
        return !userRelatedGroupIds.includes(
  ...
  isUserGrantedPageAccess(
    ...
    if (page.grant === PageGrant.GRANT_USER_GROUP)
      return (
        this.getUserRelatedGrantedGroupsSyncronously(userRelatedGroups, page)
          .length > 0
      );
    return false;
  }
  ...
  async canOverwriteDescendants(
    ...
    return this.calcCanOverwriteDescendants(
      operatorGrantInfo,
      updateGrantInfo,
      descendantPagesGrantInfo,
    );
  }
  ...
  async generateUpdateGrantInfoToOverwriteDescendants(
    ...
    if (updateGrant === PageGrant.GRANT_PUBLIC) {
      ...
      const {
        grantedUserGroups: grantedUserGroupIds,
        grantedExternalUserGroups: grantedExternalUserGroupIds,
      } = divideByType(grantGroupIds);

      const userGroupUserIds =
        await UserGroupRelation.findAllUserIdsForUserGroups(
          grantedUserGroupIds,
        );
      const externalUserGroupUserIds =
      ...
      const childrenOrItselfGroups = [
        ...childrenOrItselfUserGroups,
        ...childrenOrItselfExternalUserGroups,
      ];
  ...
  private calcIsAllDescendantsGrantedByOperator(
    ...
    if (descendantPagesGrantInfo.grantSet.has(PageGrant.GRANT_OWNER)) {
      const isNonApplicableOwnerExist =
        descendantPagesGrantInfo.grantedUserIds.size >= 2 ||
        !includesObjectIds(
          [...descendantPagesGrantInfo.grantedUserIds],
          [operatorGrantInfo.userId],
        );
    ...
    if (descendantPagesGrantInfo.grantSet.has(PageGrant.GRANT_USER_GROUP)) {
      const isNonApplicableGroupExist =
        excludeTestIdsFromTargetIds(
          [...descendantPagesGrantInfo.grantedUserGroupIds],
          [...operatorGrantInfo.userGroupIds],
        ).length > 0;
  ...
  private calcCanOverwriteDescendants(
    ...
    const isAllDescendantsGranted = this.calcIsAllDescendantsGrantedByOperator(
      operatorGrantInfo,
      descendantPagesGrantInfo,
    );
    ...
    if (updateGrantInfo.grant === PageGrant.GRANT_USER_GROUP) {
      const isAllDescendantGroupsChildrenOrItselfOfUpdateGroup =
        excludeTestIdsFromTargetIds(
          [...descendantPagesGrantInfo.grantedUserGroupIds],
          [...updateGrantInfo.grantedUserGroupInfo.childrenOrItselfGroupIds],
        ).length === 0; // a.
      const isUpdateGroupUsersIncludeAllDescendantsOwners =
        excludeTestIdsFromTargetIds(
          [...descendantPagesGrantInfo.grantedUserIds],
          [...updateGrantInfo.grantedUserGroupInfo.userIds],
        ).length === 0; // b.
      return (
        isAllDescendantGroupsChildrenOrItselfOfUpdateGroup &&
        isUpdateGroupUsersIncludeAllDescendantsOwners
      );
    }
