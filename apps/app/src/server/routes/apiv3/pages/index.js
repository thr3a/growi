...
module.exports = (crowi) => {
  ...
  const validator = {
    recent: [
      ...
      query('includeWipPage')
        .optional()
        .isBoolean()
        .withMessage('includeWipPage must be boolean'),
    ],
    renamePage: [
      body('pageId').isMongoId().withMessage('pageId is required'),
      body('newPagePath')
        .isLength({ min: 1 })
        .withMessage('newPagePath is required'),
      body('isRecursively')
        .if((value) => value != null)
        .isBoolean()
    ...
    deletePages: [
      body('pageIdToRevisionIdMap')
        .exists()
        .withMessage(
          'The body property "pageIdToRevisionIdMap" must be an json map with pageId as key and revisionId as value.',
        ),
      body('isCompletely')
        .custom((v) => v === 'true' || v === true || v == null)
        .withMessage(
          'The body property "isCompletely" must be "true" or true. (Omit param for false)',
        ),
      body('isRecursively')
        .custom((v) => v === 'true' || v === true || v == null)
        .withMessage(
          'The body property "isRecursively" must be "true" or true. (Omit param for false)',
        ),
      body('isAnyoneWithTheLink')
        .custom((v) => v === 'true' || v === true || v == null)
        .withMessage(
          'The body property "isAnyoneWithTheLink" must be "true" or true. (Omit param for false)',
        ),
    ],
    legacyPagesMigration: [
      body('convertPath')
        .optional()
        .isString()
        .withMessage('convertPath must be a string'),
      body('pageIds')
        .optional()
        .isArray()
  ...
  router.get(
    ...
    async (req, res) => {
      ...
      try {
        const result = await Page.findRecentUpdatedPages(
          '/',
          req.user,
          queryOptions,
        );
        ...
        result.pages.forEach((page) => {
          if (
            page.lastUpdateUser != null &&
            page.lastUpdateUser instanceof User
          ) {
        ...
        return res.apiv3Err(
          new ErrorV3('Failed to get recent pages', 'unknown'),
          500,
        );
      }
    },
  ...
  router.put(
    ...
    async (req, res) => {
      ...
      if (!isCreatablePage(newPagePath)) {
        return res.apiv3Err(
          new ErrorV3(
            `Could not use the path '${newPagePath}'`,
            'invalid_path',
          ),
          409,
        );
      ...
      if (isUserPage(newPagePath)) {
        ...
        if (!isExistUser) {
          return res.apiv3Err(
            "Unable to rename a page under a non-existent user's user page",
          );
        }
      }
      ...
      if (isExist) {
        ...
        return res.apiv3Err(
          new ErrorV3(`${newPagePath} already exists`, 'already_exists'),
          409,
        );
      }
      ...
      try {
        ...
        if (!page.isEmpty && revisionId == null) {
          return res.apiv3Err(
            new ErrorV3('revisionId must be a mongoId', 'invalid_body'),
            400,
          );
        }
        ...
        renamedPage = await crowi.pageService.renamePage(
          page,
          newPagePath,
          req.user,
          options,
        ...
        logger.error(err);
        return res.apiv3Err(
          new ErrorV3('Failed to update page.', 'unknown'),
          500,
        );
      }
      ...
      try {
        ...
        await globalNotificationService.fire(
          GlobalNotificationSettingEvent.PAGE_MOVE,
          renamedPage,
          req.user,
          {
            oldPath: page.path,
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const pageOp =
        await crowi.pageOperationService.getRenameSubOperationByPageId(
          page._id,
        );
      if (pageOp == null) {
        const msg =
          'PageOperation document for Rename Sub operation not found.';
  ...
  router.delete(
    ...
    async (req, res) => {
      ...
      const deletablePages = crowi.pageService.filterPagesByCanDeleteCompletely(
        pagesInTrash,
        req.user,
        true,
      );
      ...
      if (deletablePages.length < pagesInTrash.length) {
        try {
          ...
          await crowi.pageService.deleteMultiplePages(
            deletablePages,
            req.user,
            options,
          );
      ...
      // when all pages are deletable
        ...
        try {
          ...
          const pages = await crowi.pageService.emptyTrashPage(
            req.user,
            options,
            activityParameters,
          );
  ...
  router.get(
    ...
    async (req, res) => {
      ...
      try {
        const result = await Page.findListWithDescendants(
          path,
          req.user,
          queryOptions,
        );
  ...
  router.post(
    ...
    apiV3FormValidator,
    async (req, res) => {
      const { pageId, isRecursively, onlyDuplicateUserRelatedResources } =
        req.body;
      ...
      if (isUserPage(newPagePath)) {
        ...
        if (!isExistUser) {
          return res.apiv3Err(
            "Unable to duplicate a page under a non-existent user's user page",
          );
        }
      }
      ...
      if (isExist) {
        return res.apiv3Err(
          new ErrorV3(`Page exists '${newPagePath})'`, 'already_exists'),
          409,
        );
      }
  ...
  router.get(
    ...
    async (req, res) => {
      ...
      try {
        ...
        const result = await Page.findManageableListWithDescendants(
          pageData,
          req.user,
          { limit },
        );
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      if (pageIds.length === 0) {
        return res.apiv3Err(
          new ErrorV3('Select pages to delete.', 'no_page_selected'),
          400,
        );
      }
      if (isAnyoneWithTheLink && pageIds.length !== 1) {
        return res.apiv3Err(
          new ErrorV3(
            'Only one restricted page can be selected',
            'not_single_page',
          ),
          400,
        );
      ...
      if (pageIds.length > LIMIT_FOR_MULTIPLE_PAGE_OP) {
        return res.apiv3Err(
          new ErrorV3(
            `The maximum number of pages you can select is ${LIMIT_FOR_MULTIPLE_PAGE_OP}.`,
            'exceeded_maximum_number',
          ),
          400,
        );
      }
      ...
      let pagesToDelete;
      try {
        pagesToDelete = await Page.findByIdsAndViewer(
          pageIds,
          req.user,
          null,
          true,
      ...
      if (
        ...
        return res.apiv3Err(
          new ErrorV3('The grant of the retrieved page is not restricted'),
          500,
        );
      }
      ...
      if (isCompletely) {
        pagesCanBeDeleted =
          await crowi.pageService.filterPagesByCanDeleteCompletely(
            pagesToDelete,
            req.user,
            isRecursively,
          );
      ...
      crowi.pageService.deleteMultiplePages(
        pagesCanBeDeleted,
        req.user,
        options,
        activityParameters,
      ...
      return res.apiv3({
        paths: pagesCanBeDeleted.map((p) => p.path),
        isRecursively,
        isCompletely,
      });
    },
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      try {
        if (isRecursively) {
          await crowi.pageService.normalizeParentByPageIdsRecursively(
            pageIds,
            req.user,
          );
        } else {
  ...
  router.get(
    ...
    async (req, res) => {
      try {
        ...
        const migratablePagesCount =
          req.user != null
            ? await crowi.pageService.countPagesCanNormalizeParentByUser(
                req.user,
              )
            : null; // null check since not using loginRequiredStrictly
