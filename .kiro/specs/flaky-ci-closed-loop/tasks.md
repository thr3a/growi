...
- [x] 6.2 修正の検証と Ready PR を実在の修正で通す
  ...
  - _Depends: 3.2, 5.1_
  - _Requirements: 6.4, 6.5_
- [x] 6.3 routine を 1 サイクル回し、選択・自動クローズ・ダッシュボードを確認する
...
- 6.3 で見つかった手順書の不具合 10 件（D1〜D10、原資料 `scratchpad/routine-6.3/defects.md`）: **D9** 停止手順が「コメント→ラベル」の順なのにダッシュボードは「ラベル以降のコメント」を読むため、正しく止めた issue ほど `(may be stale)` が付く → ラベル先行に変更＋120 秒の許容幅。**D7** ④ 履歴バックフィルが denylist と「PR 自身の失敗」除外を通らず、#11707 を格上げして自動クローズを阻む → 両フィルタ通過後のログのみ対象。**D6** Playwright の精密識別の条件が `0 failed / 1 flaky`（最も普通の形）で成立しない（再試行で通ったテストにも `::error` 注釈が出る）→ 注釈と retry 添付の突き合わせに再定義。**D3** `gh run view --job --log-failed` は別 attempt を指定しても最新 attempt のログを黙って返す → `gh api --allow-escape-sequences .../jobs/{id}/logs`。**D2** 2-C の「stop」の範囲 → Step 3 だけ飛ばす。**D4** denylist の粒度 → FAIL ブロック単位（共有 setup フック内の一致だけジョブ全体）。**D8** Fix PR マーカーの完全一致が署名付きの旧マーカーを取りこぼす → いずれかの行が完全一致。**D1** zsh の `echo` が JSON の `\n` を展開 → `printf '%s\n'`。**D5** Playwright ジョブ名は再利用 workflow 名が前置される → `contains`。**D10** 表の行順を常時規定。レビュー 4 往復で、D6 の初版（添付パスとの突き合わせ）は実ログで成立しないと判明し「注釈 1 件 ＋ `N flaky`+`N failed`=1」（#11903 で実際に効いた規則）に置き換え、識別キーの解析は「拡張子直後の `:`」を境界にする正規表現に（共有 setup フックの `.ts` も通る、65 件の実 issue 題名で全件一致を実測）
- 6.3 2 サイクル目（実施結果、2026-09-14 20:05Z に #11849 へ人として「決定的原因として閉じる」判断を投稿 → 20:4x〜21:0xZ に `--window-hours=8` で実行）: 選択 B = #11849 のみ（Paused at 15:57:40Z、人のコメント 20:05:29Z、`user.type=User` かつ署名なし）。他の判断待ち 13 件（#11914 含む）は非選択。investigate は `flaky/needs-decision` を最初に削除 → Step 4 のゲートに人の判断を適用 → Step 3 の決定的原因クローズ（`### Closed: deterministic cause, not flaky`、`⏏ phase/wontfix`、`flaky/suspected` は保持、`Fixed by` なし、`not_planned`）。ダッシュボードは表 14 行・判断待ち 13 行（#11849 消失）・`(may be stale)` 0 件。**120 秒の許容幅が実際に効いた**: 1 サイクル目（D9 修正前の手順）で止めた #11914 はコメント→ラベルの逆順（−1 秒）で、幅が無ければ `(may be stale)` が付いていた。Fix PR 列は行単位照合で #11821/#11851/#11858 の 3 件が埋まった（D8 の効果）。新たな不備 2 件: (1) 再開の答えが「閉じる」のとき、選んだ選択肢の記録先が未定義（今回はクローズコメント内に 1 段落で記録）→ 是正、(2) detect のひな型 4 種が GraphQL 系の `gh issue comment/create/edit/reopen` のまま → REST に統一
- 7.1（実施結果）: 元 spec の requirements.md に 6.1〜11.3 を本文同一で末尾追記、2.2/2.3/3.5 は ID を保って現在の契約に書き直し、2.7（測定できなかった場合）を Requirement 2 の末尾に追加。design.md は 12 節を現在の事実で書き直し。レビューで追加: 再現 workflow は matrix の 1 セル（MongoDB 8.0 / ES 8）しか測らない制約（Requirement 6 の既知の残課題にも記載）、Fix PR 行単位照合、切り詰め時も並び順は固定。**記録**: Requirement 7.1/7.3 の文言は「同一 run」だが、実装と design は「同一ジョブログ」を範囲とする（承認済み要件本文は変えない方針のため据え置き）。investigate の amend spec 名参照 2 か所は元 spec へ付け替え済み。残る素の task 番号参照（investigate 2 か所「task 1.6 measured」「task 1.1 made deliberate」、flaky-repro.yml「task 1.3」）は 7.3 で扱う
